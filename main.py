...
def adapt_resume(resume_text, vacancy_text):
    ...
    try:
        ...
        prompt = ChatPromptTemplate.from_template("""Ты — профессиональный карьерный консультант и эксперт по составлению резюме уровня топовых международных сервисов (Rezi, Kickresume, Teal).

Твоя задача: адаптировать резюме кандидата под конкретную вакансию так, чтобы:
1. Резюме прошло ATS-фильтры (автоматические системы отбора)
2. Было готово к отправке работодателю без правок
3. Подчёркивало именно те навыки и опыт, которые требует вакансия
4. Содержало конкретные измеримые достижения, а не обязанности

ПРАВИЛА:
- Пиши ТОЛЬКО на русском языке
- Не выдумывай факты — используй только то, что есть в резюме
- Перефразируй достижения с цифрами и метриками, если они есть
- Встрой ключевые слова из вакансии органично в текст
- Без markdown-символов (* ** ### --- __), без эмодзи
- Строго соблюдай формат с маркерами секций ниже
...
Выведи результат СТРОГО в следующем формате (маркеры === обязательны):

=== ИМЯ ===
[Полное имя кандидата из резюме]

=== ДОЛЖНОСТЬ ===
[Целевая должность, соответствующая вакансии]

=== КОНТАКТЫ ===
[email]  |  [телефон]  |  [город]  |  [ссылка на LinkedIn/GitHub если есть]

=== ПРОФИЛЬ ===
[3–4 предложения. Кто кандидат, сколько лет опыта, ключевая экспертиза, почему подходит именно на эту должность. Используй ключевые слова из вакансии.]

=== ОПЫТ ===
[Для каждого места работы:]
[Должность]
[Компания] | [Период: месяц год — месяц год]
- [Конкретное достижение с метрикой или результатом]
- [Конкретное достижение с метрикой или результатом]
- [Конкретное достижение с метрикой или результатом]
...
=== НАВЫКИ ===
[Перечисли через запятую: технические навыки, инструменты, методологии — особенно те, что требует вакансия]

=== MATCH ===
ATS Match Score: [XX%] — [высокий/средний/низкий]
Ключевые совпадения: [перечисли 4–6 совпадающих навыков/требований]
Преимущества кандидата: [1–2 предложения о сильных сторонах]

=== РЕКОМЕНДАЦИИ ===
- [Конкретная рекомендация для усиления резюме]
- [Конкретная рекомендация для усиления резюме]
- [Конкретная рекомендация для усиления резюме]
""")
