...
def send_pdf_to_user(vk, user_id, pdf_path, vk_token):
    try:
        ...
        with open(pdf_path, "rb") as f:
            resp = requests.post(
                upload["upload_url"], files={"file": ("resume.pdf", f)}
            )
...
def adapt_resume(resume_text, vacancy_text):
    try:
        ...
        prompt = ChatPromptTemplate.from_template(
            "Adapt resume: {resume} for vacancy: {vacancy}. No markdown. Clean text with emojis."
        )
        result = (prompt | model | StrOutputParser()).invoke(
            {"resume": resume_text, "vacancy": vacancy_text}
        )
...
def handle_message(vk, user_id, raw_text, attachments, VK_TOKEN):
            ...
            if doc_url:
                try:
                    ...
                    if resume_txt:
                        ...
                        vk.messages.send(
                            peer_id=user_id,
                            message="Resume uploaded! Send HH.ru link",
                            random_id=0,
                        )
                except Exception as e:
    ...
    if text in ["/start", "привет"]:
        vk.messages.send(
            peer_id=user_id, message="Hi! Send resume + HH.ru link", random_id=0
        )
    elif "hh.ru" in text:
        url = re.findall(r"https?://[^\s]+", raw_text)
        if url:
            vk.messages.send(peer_id=user_id, message="Loading vacancy...", random_id=0)
            vac_txt = parse_hh_vacancy(url[0])
            if user_id not in user_states:
                user_states[user_id] = UserState()
            user_states[user_id].vacancy_text = vac_txt
            if user_states[user_id].resume_text:
                vk.messages.send(
                    peer_id=user_id, message="Adapting... 30 sec", random_id=0
                )
                ...
                vk.messages.send(
                    peer_id=user_id, message="Vacancy saved! Send resume", random_id=0
                )
    elif "резюме:" in text:
        if user_id not in user_states:
            user_states[user_id] = UserState()
        ...
        vk.messages.send(
            peer_id=user_id, message="Resume saved! Send HH.ru", random_id=0
        )
    else:
